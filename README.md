...
| `E2E (kind, alpha-beta-features)` | [![E2E (kind, alpha-beta-features)](https://github.com/dims/test-k8s/actions/workflows/e2e-kind-alpha-beta-features.yml/badge.svg?branch=main)](https://github.com/dims/test-k8s/actions/workflows/e2e-kind-alpha-beta-features.yml) | Scheduled/manual off-by-default alpha and beta feature coverage in kind with all APIs enabled | `e2e-k8s.sh` |
...
- The main CI lanes (`Verify`, `Unit Tests`, `Integration Tests`, `Typecheck`, `Dependencies`, `Cmd Tests`, `Linter Hints`, `E2E (kind)`, `Conformance (kind, GA-only)`, and `Node E2E`) run on pushes to `main` and pull requests to `main` when relevant workflow/action/config files change, on manual `workflow_dispatch`, and every 6 hours.
- `E2E (kind, alpha-beta-features)` runs on `workflow_dispatch` and a daily schedule.
