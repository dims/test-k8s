...
**Fix:** Introduce `allScheduled := true` in the poll closure. For every non-terminal pod that has an empty `spec.nodeName`, set `allScheduled = false`. Change the return condition from `nonTerminatedPods == numberPods` to `nonTerminatedPods == numberPods && allScheduled`. This forces the poll to continue until every pod has been scheduler-bound before returning, guaranteeing that `UpdateStatus` is subsequently called and `NumberReady` reaches the expected value.  
